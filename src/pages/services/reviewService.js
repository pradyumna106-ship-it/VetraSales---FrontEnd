...
export const listOfReview = async (productId) => { 
    const response = await axios.get(`${BASE_URL}/listOfReview`,productId,{headers: { "Content-Type": "application/json" }});
...
export const allReviews =  async () => {
    const response = await axios.get(`${BASE_URL}/allReviews`,{headers: { "Content-Type": "application/json" }});
    return response.data;
};